/** \mainpage Driver Overview
  ...
  * Studio tool:
  * - <b>Project name</b>:     LeakADC
  * - <b>Project file</b>:     Z:/Ценные файлы/Ti/LeakADC/LeakADC.scp
  * - <b>Code prefix</b>:      -
  ...
  * - <b>Created</b>:          2017-11-11 18:51:03.275
  ...
  *
  *
  * \subsection section_task_desc_leak_adc LeakADC
  * No description entered
  *
  */
...
/// LeakADC: Task ID
#define SCIF_LEAK_ADC_TASK_ID 0


/// LeakADC I/O mapping: ADC Input
#define SCIF_LEAK_ADC_DIO_A_ADC_INPUT 29
/// LeakADC I/O mapping: Sensor power
#define SCIF_LEAK_ADC_DIO_O_SENSOR_POWER 7
...
/// LeakADC: Task configuration structure
typedef struct {
    uint16_t continuousAlarmPeriod; ///< 
    uint16_t continuousThreshold;   ///< 
    uint16_t instantThreshold;      ///< 
} SCIF_LEAK_ADC_CFG_T;


/// LeakADC: Task output data structure
typedef struct {
    uint16_t adcValue; ///< 
} SCIF_LEAK_ADC_OUTPUT_T;


/// LeakADC: Task state structure
typedef struct {
    uint16_t alert;                      ///< alert
    uint16_t continuousThresholdCounter; ///< 
    uint16_t instantThresholdCounter;    ///< 
} SCIF_LEAK_ADC_STATE_T;


/// Sensor Controller task data (configuration, input buffer(s), output buffer(s) and internal state)
typedef struct {
    struct {
        SCIF_LEAK_ADC_CFG_T cfg;
        SCIF_LEAK_ADC_OUTPUT_T output;
        SCIF_LEAK_ADC_STATE_T state;
    } leakAdc;
} SCIF_TASK_DATA_T;
